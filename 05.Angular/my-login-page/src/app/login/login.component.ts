...
export class LoginComponent {
  ...
  signIn(form: NgForm) {
    ...
    // 6 karakter kontrolü
    const passwordValue: string = form.controls["password"].value;
    const isPasswordValid: boolean = passwordValue.length >= 6;

    if (form.valid && isValid && isPasswordValid) {
      console.log(form);
      ...
      if (form.valid && isValid === true && isPasswordValid) {
