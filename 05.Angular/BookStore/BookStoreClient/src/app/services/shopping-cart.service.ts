...
export class ShoppingCartService {
  ...
  constructor(private swal: SwalService, private translate: TranslateService ,private http:HttpClient,private ngxSpinner:NgxSpinnerService,
  ...
  changeBookQuantityInShoppingCart(bookId:Number,quantity:number){
    if(localStorage.getItem("response")){
      this.http.get(`https://localhost:7215/api/ShoppingCarts/ChangeBookQuantityInShoppingCart/${bookId}/${quantity}`)
      .subscribe({
        next:(res:any)=>{
      this.getAllShoppingCarts();
      this.calcTotal();
      ...
      if(quantity<=0){
        ...
        this.http.get(`https://localhost:7215/api/ShoppingCarts/CheckBookQuantityIsAvailable/${bookId}/${quantity}`)
        .subscribe({
          next:(res:any)=>{
  ...
  getAllShoppingCarts(){
    ...
    if(localStorage.getItem("response"))
    {
      ...
      this.http.get<SetShoppingCartsModel[]>("https://localhost:7215/api/ShoppingCarts/GetAll/"+this.auth.userId).subscribe({
        next:(res:any)=>  {
  ...
  removeByIndex(index: number) {
    forkJoin({
      ...
      this.swal.callSwall(res.doYouWantToDeleted,res.cancelBtn,res.confirmBtn,() => {
        if(localStorage.getItem("response")){
          this.http.get("https://localhost:7215/api/ShoppingCarts/RemoveById/"+this.shoppingCarts[index]?.shoppingCartId).subscribe(res=>{
            this.getAllShoppingCarts()
          })
        }
        else{
  ...
  payment(data:PaymentModel,callBack:(res:any)=>void){
    ...
    this.http.post("https://localhost:7215/api/ShoppingCarts/Payment",data)
    .subscribe({
      next:(res:any)=>{
        callBack(res)
